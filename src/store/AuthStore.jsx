...
export const useAuthStore = create((set) => ({ // Store para manejar la autenticación
    ...
    crearUserYLogin: async (p) => { // Función para crear un usuario y iniciar sesión
        const {data} = await supabase.auth.signUp({ // Llama a la API de Supabase para crear un usuario
            email: p.email,
            password: p.password 
        })
        return data.user;
